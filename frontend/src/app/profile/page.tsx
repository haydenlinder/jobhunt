    ...
    return (
      <div className="bg-red-50 border-l-4 border-red-500 p-4 my-4">
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm text-red-700">Failed to load profile data. Please try again later.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {!isEmployer && (
            <div>
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Resume & Skills</h3>
          ...
          {isEmployer && profileData?.company_users && (
            <div>
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Your Organizations</h3>
              <div className="space-y-4">
                {profileData.company_users.map(companyUser => (
                  <div key={companyUser.id} className="bg-gray-50 dark:bg-gray-700 p-4 rounded-md">
                    ...
                    <div className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                      <div>Job Postings: {companyUser.company?.jobs_aggregate?.aggregate?.count || 0}</div>
                    </div>
                  </div>
                ))}
              </div>
