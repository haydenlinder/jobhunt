...
export default function FeaturedCompanies() {
  const { data=[], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const data = await graphqlRequest<GetFeaturedCompaniesQuery>(GET_FEATURED_COMPANIES.loc?.source.body||"");
  ...
  return (
    <section>
      <div className="flex justify-between items-baseline mb-6">
        ...
        <Link href="/companies" className="text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">
          View all companies →
        </Link>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          Array(3).fill(0).map((_, index) => (
            <div key={index} className="block bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
